...
function compareArrays(arr1, arr2) {
  ...
  let arr1Sort = arr1.sort((value1, value2) => value1 - value2);
  let arr2Sort = arr2.sort((value1, value2) => value1 - value2);

  console.assert(
    JSON.stringify(arr1Sort) === JSON.stringify(arr2Sort),
    "Масиви не однакові!"
  );
}
...
function buildMatrix(size) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < size; j++)
      rowMatrix.push(Math.floor(Math.random() * 10));
...
function calculateTotalPrice(products) {
  ...
  let total = products.reduce((sum, value) => sum + value.price, 0);
...
function countVowelsAndConsonants(word) {
  ...
  for (let letter of word) {
    if (str.includes(letter)) console.count("Голосна");
